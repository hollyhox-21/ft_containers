...
namespace ft{
	...
	private:
		...
		void	_clean_segment(value_type *&arr) {
			_alloc_data.destroy(arr + _size - 1);
			_size--;
		}
		...
		void	_move_right(iterator position) {
			...
			if (position == it_e)
				return ;
			it_e--;
			_alloc_data.construct(_array + _size, 0);
...
//		}
	public:
		...
		_size(0), _capacity(0), _alloc_data(alloc) {
			_array = _alloc_data.allocate(_capacity);
		...
		~vector() {
			clear();
			if (_capacity)
				_alloc_data.deallocate(_array, _capacity);
		}
...
//==================>>>Modifiers<<<====================
		...
		iterator insert (iterator position, const value_type& val) {
			if (_size < _capacity) {
				...
				difference_type n = position._cure_item - _array;
				_realloc_memory(_capacity * 2);
				position = begin() + n;
				_move_right(position);
				_insert_obj(position, val);
			}
			else if (_size == 0 && _capacity == 0) {
				_insert_obj(val);
				position = begin();
			}
			return position;
